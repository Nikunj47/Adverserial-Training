...
class TransformerInferenceGraph(TransformerTrainGraph):
  ...
  """
    ...
    def decoder_body(self, enc, dec, is_training, top_scope=None):
        ...
        decoded_logits = tf.zeros([logits_bs, 0, config.n_labels], dtype=tf.float32)  #### FOUND LOGITS

        self.adversarial = self.attack(self, decoded_logits)  ### Calls attack
    ...
    def attack(self, decoded_logits):
        label = tf.argmax(decoded_logits, 1)
        one_hot_label = tf.one_hot(label, config.n_labels)
        cross_entropy = tf.losses.softmax_cross_entropy(one_hot_label,
                                                        decoded_logits,
                                                        label_smoothing=0.1,
                                                        weights=1.0)
        adv = self.x + 0.05 * tf.sign(tf.gradients(cross_entropy, self.x)[0])
